...
SELECT lname, author_id, manuscript_id
FROM Author
       JOIN Authorship
...
SELECT Concat(fname, " ", lname) AS name, author_id, title
FROM Author
       JOIN Authorship
            ON Authorship.author_id = Author.id
       JOIN Manuscript
...
SELECT Concat(fname, " ", lname)                             AS primary_author_name,
       ...
       GROUP_CONCAT((
                      ...
                      WHERE Manuscript.id = Authorship.manuscript_id
                      GROUP BY manuscript_id) SEPARATOR ',') AS reviewer_names
FROM Manuscript
       JOIN Authorship
...
SELECT id, man_status, status_last_updated
FROM Manuscript;
